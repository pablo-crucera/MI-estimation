...
# Bootstrap
B <- 200
N_sizes <- c(50, 100, 200, 500, 1000, 2000, 3000)
quantile2.5 <- NULL
quantile97.5 <- NULL
bootstrap_means <- NULL
orig_pop_estimates <- NULL
bias <- NULL
...
for (N in N_sizes) {
  ...
  orig_pop_estimates <- c(orig_pop_estimates, MI_estimation(df$X, df$Y, 'LNC'))
  ...
  bootstrap_means <- c(bootstrap_means, mean(bootstrap_estimates))
  bias <- c(bias, mean(bootstrap_estimates) - orig_pop_estimates[length(orig_pop_estimates)])
  quantile2.5 <- c(quantile2.5, quantile(bootstrap_estimates, 0.025))
  quantile97.5 <- c(quantile97.5, quantile(bootstrap_estimates, 0.975))
}


# Definition of a function that wraps all existing methods
